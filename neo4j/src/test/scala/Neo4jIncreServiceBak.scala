...
object Neo4jIncreServiceBak {
  def main(args: Array[String]): Unit = {
    ...
    val yestoday = LocalDateTime.now().minusDays(1).toString().substring(0, 10)
    ...
    callDetailRDD.saveAsTextFile(savePath + s"/$yestoday/relationship")

    callDetailRDD.map(x => {
      val sp = x.split(",")
      sp(0) + "," + sp(1)
    }).flatMap(_.split(",")).distinct()
      .saveAsTextFile(savePath + s"/$yestoday/person")
  }
}
