...
/**
  ...
  * 实现:
  *    1. 在bdw_tinyv_outer.b_fuse_call_detail_i通话详单中取出关系 relationship.csv
  ...
  */
object Neo4jCallDetailCreditBak02 {
  def main(args: Array[String]): Unit = {
    ...
    val mobileCredit: List[String] = passCredit.map(_.getAs[String](0)).collect().toList
    val bmc = spark.sparkContext.broadcast(mobileCredit)
    ...
    val callDetailRDD = spark.sql(callDetailSql).map(row => {
      ...
      row.getAs[String](2) match {
        case "012001001" =>
          if (mc.contains(mobile) && mc.contains(other_mobile)) {
            (mobile + "," + other_mobile + ",CALL_OUT", call_time)
          } else {
            ("1", 1.0)
          }
        case "012001002" =>
          if (mc.contains(mobile) && mc.contains(other_mobile)) {
            (other_mobile + "," + mobile + ",CALL_OUT", call_time)
          } else {
            ("1", 1.0)
          }
      }
    }).filter(x => {
      val phonePattern = "^[0-9]{11},[0-9]{11},CALL_OUT".r
      x._1 match {
...
//    val callDetailSql =
//      s"""
//         |select c.mobile as mobile,c.other_mobile as other_mobile from
//         |(select
//         |b.mobile as mobile,
...
//         |join passCredit on passCredit.mobile=c.other_mobile
//      """.stripMargin
  }
}
