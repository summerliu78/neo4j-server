...
/**
  ...
  *       2.1 在我司申请授信未通过的人　applyNotPass.csv
  *       2.2 在我司申请授信未借款用户 applycreditNoBorrow.csv
  *       2.3 借款且逾期天数小于7天(白名单) whitelist.csv
  *       2.4 逾期天数大于15天(黑名单) blacklist.csv
  *       2.5 借款但是既不是白名单也不是黑名单那部分人 graylist.csv
  *
  *    2. 在bdw_tinyv_outer.b_fuse_call_detail_i通话详单中取出关系 relationship.csv
  * 这个关系是授过信的人的通话关系,都没有在我司借款的人不算在内
  */
object Neo4jCallDetailCreditBak04 {
  def main(args: Array[String]): Unit = {
    ...
    val applyNotPassSql =
      s"""
         ...
         |where dt='$pass_day'
         |and FIRST_APPLY_CREDIT_TIME is not null
         |and FIRST_APPLY_CREDIT_SUCC_TIME is null) a
         |join
    ...
    // 申请授信没有通过的
    spark.sql(applyNotPassSql).map(_.getAs[String](0)).coalesce(1).distinct()
      .write.text(savePath + s"/$pass_day/applyNotPass")
    ...
    val applyPassSql =
      s"""
         ...
         |from
         |(select USER_ID,ds,FIRST_APPLY_CREDIT_SUCC_TIME,FIRST_APPLY_LOAN_SUCC_TIME,
         |MAX_OVERDUE_DAY_CNT,CURR_INLOAN_CNT
         |from
         |edw_tinyv.e_user_integ_info_d
    ...
    val applyCreditSql =
      s"""
         ...
         |edw_tinyv.e_user_integ_info_d
         |where dt='$pass_day'
         |and FIRST_APPLY_CREDIT_TIME is not null) a
         |join
         |(select USER_ID,ds,MOBILE from ods_tinyv.o_fuse_user_info_contrast_d
         |where dt='$pass_day') b
         |on a.USER_ID=b.USER_ID and a.ds=b.ds
      """.stripMargin
    spark.sql(applyCreditSql).createOrReplaceTempView("applyCredit")

    // 各个节点之间的关系
    val callDetailRelsSql =
      s"""
         |select
         |c.mobile as mobile,
    ...
    spark.sql(callDetailRelsSql).map(row => {
